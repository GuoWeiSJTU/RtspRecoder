...
struct MediaMetadata {
    std::unique_ptr<AVCodecParameters, std::function<void(AVCodecParameters*)>> video_codec_params;
    std::unique_ptr<AVCodecParameters, std::function<void(AVCodecParameters*)>> audio_codec_params;
    int width = 0;
    int height = 0;
    int sample_rate = 0;
    
    MediaMetadata() 
        : video_codec_params(nullptr, [](AVCodecParameters* p) { avcodec_parameters_free(&p); }),
          audio_codec_params(nullptr, [](AVCodecParameters* p) { avcodec_parameters_free(&p); }) {}
};
